...
function [ ] = plotResults( params, out, obs )
...
%%% Plot the results
% Initialize
f1 = figure(1);pos = get(f1,'position');set(gcf,'color','w');
...
% Save
exportgraphics(f1,'./output/ch4_simulated.png'); %,'-png','-m2','-painters','-cmyk')
...
set(h5,'Position',[.13,.06,.7750,.16])
% Save
exportgraphics(f2,'./output/ch4_drivers.png'); %,'-png','-m2','-painters','-cmyk')
...
% Save
exportgraphics(f3,'./output/ch4_lgm.png'); %,'-png','-m2','-painters','-cmyk')
