...
plot(iceNHCH4.t, iceNHCH4.CH4, '.','Color','red');
plot(St, obs.ch4_NH, '.', 'Color', 'black', 'MarkerSize', 10)  % these are the block averaged data
ylabel('NH CH_4 (ppb)');
...
plot(d13c.t,d13c.d13c,'.','Color','red');
plot(St,obs.d13c,'.','Color','black', 'MarkerSize',10);
...
plot(dD.t,dD.dD,'.','Color','red');
plot(St,obs.dD,'.','Color','black', 'MarkerSize',10);
% for i = 1:length(dD.t)
...
plot(d14c.t,d14c.d14c,'.','Color','red');
plot(St,obs.d14c,'.','Color','black', 'MarkerSize',10);
...
% % plotting temp to look at DO events
% htemp = subplot(7,1,6);
% hold on; box on;
% set(gca,'YDir','normal','FontName','Helvetica','FontSize',16,'TickDir','out','LineWidth',2,'XMinorTick','on','YMinorTick','on')
% plot(dat.t,params.sTempN,'-','Color',emsCol(1,:),'LineWidth',3);
% set(gca,'YGrid','on','YAxisLocation','right');
% ylabel('sTemp');
% xlabel('Time (kyr BP)');
% xlim(xLims);xLL = xlim;yLL = [0,1];
% plot([xLL(1),xLL(2)],[yLL(1),yLL(1)],'-','LineWidth',2,'Color',YYCol);
% plot([xLL(2),xLL(2)],[yLL(1),yLL(2)],'-','LineWidth',2,'Color',YYCol);
% plot([xLL(2),xLL(1)],[yLL(2),yLL(2)],'-','LineWidth',2,'Color',YYCol);
% plot([xLL(1),xLL(1)],[yLL(2),yLL(1)],'-','LineWidth',2,'Color',YYCol);
...
%set(htemp,'Position',[.13,base + 2*sep + 1*len,.7750,.12]);
